...
class Scheduler:
    ...
    def __init__(self, configuration, task_instance, runtime = None, mode = 'blocking'):
    ...
    def get_scheduled_runtime(self):
        ...
        Args:
            configuration (dict): Configuration dictionary containing the 'runtime' key.
    ...
    def schedule_task_blocking(self, task_instance, task_runtime):
        ...
        try:
            if "minute" in task_runtime:
                min = int(task_runtime.split("minute")[0])
                self._scheduler.add_job(task_instance.initialize, trigger='cron', minute=f'*/{min}', timezone=utc)
    ...
    def schedule_task_asyncio(self, task_instance, task_runtime):
        """
        ...
        try:
            asyncio.run(self.run_schedule_task_async(task_instance, task_runtime))
        except (KeyboardInterrupt, SystemExit):
            self._scheduler.shutdown()
